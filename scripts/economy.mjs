...
import { ACTIVATION_TO_POOL, MODULE_ID, POOLS, getPoolState, isInCombat, isTracked, resetPools, spend } from "./state.mjs";
...
export function registerEconomyHooks() {
  ...
  // Само списание.
  if ( automatic ) on("dnd5e.postUseActivity", onPostUseActivity);
...
function onPreUseActivity(activity, usageConfig, dialogConfig, messageConfig) {
  ...
  const message = game.i18n.format("ACTION_ECONOMY.Notify.spent", { pool: label, actor: actor.name });
